...
これは **MVP完成版（フェーズ5まで）** の動くプロトタイプです。タイトル → STAGE 0〜4 → エンディングまで通しで遊べます。バグ6種（足場判定・重力・カメラ・扉・敵AI・UI実体化）の「発生 → デバッグパネル → 修正/利用 → 副作用」ループ、侵食演出①〜④、そして **プレイヤー自身が修正対象になるラスト**（2エンディング）が入っています。
