...
これは **フェーズ4まで実装した動くプロトタイプ** です。タイトル画面 → STAGE 0〜3 を通しで遊べ、バグ6種（足場判定・重力・カメラ・扉・敵AI・UI実体化）の「発生 → デバッグパネル → 修正/利用 → 副作用」ループと、侵食演出（①違和感／②UI・ログ異変／③デバッグ画面そのものの侵食）が入っています。
...
- **STAGE 4（ラスト）**＋**演出段階④**：プレイヤー自身が修正対象（`BUG: TESTER_PRESENCE`）になる回収とエンディング
