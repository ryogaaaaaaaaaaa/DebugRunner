...
これは **MVP完成版（フェーズ5まで）＋駆け引き強化** の動くプロトタイプです。タイトル → STAGE 0〜4 → エンディングまで通しで遊べます。バグ6種（足場判定・重力・カメラ・扉・敵AI・UI実体化）の「発生 → **怪しいグリッチをハンマーで修正** / 放置して利用 → 副作用」ループ、侵食演出①〜④、**プレイヤー自身がハンマーの修正対象になるラスト**が入っています。

**修正はメニューではなく世界内アクション**：不具合が起きたら**赤く点滅するグリッチに近づき、`E`（PCは`Tab`も / スマホは`⚙`）でハンマーとんとん**。脇に小さなコンソールが出てコードを直します。放置すればそのバグを利用して進めます。
...
- **◀ ▶**：移動／**JUMP**：ジャンプ／**⚙ FIX**：ハンマー修正（近くのグリッチ）／**⏸**：ポーズ
- タイトルの START・クリア後の CONTINUE/RETRY・ポーズメニューもタップ可。
...
3. `! BUG DETECTED` → 点滅するグリッチに近づき **E/Tab/⚙ でハンマー修正**（放置すれば利用）。
4. **2つの正解ルート**:
   - **FIX**: 足場を固体化して上を渡る。別の浮きブロックが壊れる（**副作用**）＋ `corruption` +1。
   - **IGNORE**: すり抜けたまま下の近道でゴールへ（バグ“利用”）。

### STAGE 1 — SHORTCUT（BUG#03 重力 ＋ BUG#05 カメラ）
- 進入すると **カメラバグ**（視界の固定が外れ、上方の隠しルートが見える）と **重力バグ**（低重力でフワフワ）が発生。地上に **2つのグリッチノード** が現れ、それぞれハンマー修正できる。
...
- 進入すると **ビルドがプレイヤーを「未登録エンティティ」として検出**（`BUG#00 : TESTER_PRESENCE`）。修正対象はプレイヤー自身。
- **自分をハンマーで叩く（E/⚙）＝FIX**：あなたがパッチアウトされる（分解演出 →「tester removed.」）。
- **出口へ歩いて拒否＝IGNORE**：「the tester is still in the build. / it is still watching.」
